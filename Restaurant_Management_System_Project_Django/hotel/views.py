...
def food_search(request):
    ...
    if query:
        ...
        foods = Food.objects.filter(course=cuisine)
    else:
...
def profile(request):
    ...
    profile = Customer.objects.all()  # Fetch profile data associated with the user
    context = {
        'user': user,
...
def menu(request):
    ...
    if cuisine is not None:
        ...
        if cuisine_lower == "gujarati":
            foods = Food.objects.filter(status="Enabled", course="Gujarati Food")
        elif cuisine_lower == "punjabi":
            foods = Food.objects.filter(status="Enabled", course="Punjabi Food")
        elif cuisine_lower == "south":
            foods = Food.objects.filter(status="Enabled", course="South Food")
        elif cuisine_lower == "fast":
            foods = Food.objects.filter(status="Enabled", course="Fast Food")
        else:
            foods = Food.objects.filter() 
    else:
        foods = Food.objects.filter()
